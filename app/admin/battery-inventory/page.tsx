...
export default function BatteryInventoryAdmin() {
  ...
  const loadData = async () => {
    try {
      ...
      setAllocationBase(result.allocationBase || 90);
      setFinalEmergencyReserve(result.finalEmergencyReserve || 142);
      setLoading(false);
    } catch (error) {
    ...
    setData(prevData => {
      const updated = prevData.map(loc => {
        if (loc.location === location) {
          const excess = Math.max(0, newInventory - allocationBase);
          return {
            ...loc,
  ...
  const handleAllocationBaseChange = (newBase: number) => {
    setAllocationBase(newBase);
    // Recalculate excess for all locations
    setData(prevData => {
      const updated = prevData.map(loc => {
        const excess = Math.max(0, loc.actualInventory - newBase);
        return {
          ...loc,
          allocationBase: newBase,
          excessBatteries: excess
        };
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        finalEmergencyReserve: finalEmergencyReserve,
        locations: data
      };
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-xs text-gray-600 mb-2">Battery Reserve 1 (Excess)</div>
                ...
                <div className="text-xs text-gray-500 mt-1">units over allocation</div>
              </div>
